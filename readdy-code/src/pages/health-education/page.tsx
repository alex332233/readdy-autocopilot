...
import { healthEducationData, healthCategories } from '../../mocks/health-education';
...
export default function HealthEducationPage() {
  ...
  const currentCategoryObj = healthCategories.find(c => c.name === activeCategory);
  ...
  const filtered = healthEducationData.filter(a => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-12 pb-20">
        <div className="max-w-6xl mx-auto px-6">
          {filtered.length === 0 ? (
            <div className="text-center py-20 text-gray-400 text-sm tracking-widest">
              此分類尚無文章，敬請期待
            </div>
          ) : (
