import { laserCases, laserDetailData } from './specializedDetailContent';
import TreatmentCasesSection from './TreatmentCasesSection';
import type { FeaturedTreatmentDetailContent, FeaturedTreatmentSection } from '../../../sanity/types';
import {
...
interface LaserDetailSectionProps {
  detail: FeaturedTreatmentDetailContent;
}

export default function LaserDetailSection({ detail }: LaserDetailSectionProps) {
  const sections = (detail.sections.length >= 4 ? detail.sections : laserDetailData.sections) as Array<
    FeaturedTreatmentSection & { subtitle?: string }
  >;
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide" data-sanity={dataAttribute('sections[0].title')}>{sections[0].title}</h3>
          </div>
          <div className="pl-12">
            <p className="text-sm lg:text-base font-semibold mb-3 tracking-wide" style={{ color: ACCENT }} data-sanity={dataAttribute('sections[0].eyebrow')}>{sections[0].eyebrow || sections[0].subtitle}</p>
            <p className="text-sm lg:text-base text-gray-600 leading-loose" data-sanity={dataAttribute('sections[0].content')}>{sections[0].content}</p>
            <div className="mt-6 flex flex-wrap gap-2">
              {['怕針族群', '兒童青少年', '焦慮型患者', '成長發育', '睡眠改善', '情緒專注力', '過敏體質', '長者'].map((tag) => (
        ...
        <div className="mb-16 grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-2xl overflow-hidden border border-[#c4d8d0] min-h-[560px]">
          <div className="bg-white px-12 py-16 lg:px-16 lg:py-20 flex flex-col justify-center">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide" data-sanity={dataAttribute('sections[1].title')}>{sections[1].title}</h3>
            </div>
            <p className="text-base text-gray-600 leading-[2.2]" data-sanity={dataAttribute('sections[1].content')}>{sections[1].content}</p>
          </div>
          <div className="relative min-h-[400px] lg:min-h-0" data-sanity-edit-group data-sanity-edit-target>
            <img src={primaryImage} alt="光能修復困擾" className="absolute inset-0 w-full h-full object-cover object-top" data-sanity={dataAttribute('primaryImage')} />
        ...
        <div className="mb-16 grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-2xl overflow-hidden border border-[#c4d8d0] min-h-[560px]">
          <div className="relative min-h-[400px] lg:min-h-0 order-2 lg:order-1" data-sanity-edit-group data-sanity-edit-target>
            <img src={secondaryImage} alt="光能解方" className="absolute inset-0 w-full h-full object-cover object-top" data-sanity={dataAttribute('secondaryImage')} />
          ...
          <div className="bg-[#f2f7f5] px-12 py-16 lg:px-16 lg:py-20 flex flex-col justify-center order-1 lg:order-2">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide" data-sanity={dataAttribute('sections[2].title')}>{sections[2].title}</h3>
            </div>
            <p className="text-base text-gray-600 leading-[2.2] mb-6" data-sanity={dataAttribute('sections[2].content')}>{sections[2].content}</p>
            <p className="text-base text-gray-600 leading-[2.2]" data-sanity={dataAttribute('sections[2].additionalContent')}>{sections[2].additionalContent}</p>
          </div>
        </div>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide" data-sanity={dataAttribute('sections[3].title')}>{sections[3].title}</h3>
          </div>
          <p className="text-sm text-gray-500 pl-12 mb-8">在艾苜，每一次的光能修復，都有我們嚴謹的流程把關：</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {sections[3].items.map((item, itemIdx) => (
              <div key={itemIdx} className="bg-white rounded-xl border border-[#c4d8d0] p-6 hover:border-[#5a7a6e]/50 transition-all duration-200">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-base font-semibold text-gray-800 mb-2 tracking-wide" data-sanity={dataAttribute(`sections[3].items[${itemIdx}].subtitle`)}>{item.subtitle}</h4>
                    <p className="text-sm text-gray-600 leading-relaxed" data-sanity={dataAttribute(`sections[3].items[${itemIdx}].text`)}>{item.text}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-[#f2f7f5] rounded-xl p-6 border border-[#c4d8d0]" style={{ borderLeftWidth: '4px', borderLeftColor: ACCENT }}>
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs lg:text-sm text-gray-500 leading-relaxed" data-sanity={dataAttribute('disclaimer')}>{detail.disclaimer || laserDetailData.disclaimer}</p>
          </div>
        </div>
