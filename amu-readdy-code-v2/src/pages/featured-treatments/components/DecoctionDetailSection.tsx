      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8">
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.audience.title}</h3>
          </div>
          <div className="pl-12">
            <p className="text-sm lg:text-base text-gray-600 leading-loose">{d.audience.content}</p>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.comparison.title}</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pl-0">
            {d.comparison.items.map((item, i) => (
              <div key={i} className="bg-white rounded-xl border border-[#c4d8d0] p-6 flex flex-col gap-4">
                ...
                <h4 className="text-base font-bold text-gray-800 tracking-wide" style={{ fontFamily: "'Noto Serif TC', serif" }}>{item.title}</h4>
        ...
        <div className="mb-16 grid grid-cols-1 lg:grid-cols-2 gap-0 rounded-2xl overflow-hidden border border-[#c4d8d0] min-h-[560px]">
          <div className="relative min-h-[400px] lg:min-h-0 order-2 lg:order-1">
            <img src={secondaryImage} alt="艾苜水煎藥解方" className="absolute inset-0 w-full h-full object-cover object-top" />
          ...
          <div className="bg-[#f2f7f5] px-12 py-16 lg:px-16 lg:py-20 flex flex-col justify-center order-1 lg:order-2">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.solution.title}</h3>
            </div>
            <p className="text-sm text-gray-500 mb-6 leading-relaxed">{d.solution.intro}</p>
            <div className="space-y-4">
              {d.solution.items.map((item, i) => (
                <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white border" style={{ borderColor: `${ACCENT}25` }}>
                  ...
                  <div>
                    <p className="text-sm font-bold mb-1 text-gray-800">{item.title}</p>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.keyStages.title}</h3>
          </div>
          <p className="text-sm text-gray-500 pl-12 mb-3">{d.keyStages.subtitle}</p>
          <p className="text-sm lg:text-base text-gray-600 leading-loose pl-12 mb-8">{d.keyStages.intro}</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {d.keyStages.items.map((item, i) => (
              <div key={i} className={`bg-white rounded-xl border border-[#c4d8d0] p-6 hover:border-[#5a7a6e]/50 transition-all duration-200 ${i === 4 ? 'md:col-span-2 lg:col-span-1' : ''}`}>
                <h4 className="text-base font-semibold text-gray-800 mb-2 tracking-wide" style={{ fontFamily: "'Noto Serif TC', serif" }}>{item.title}</h4>
                <p className="text-sm text-gray-600 leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.beyond.title}</h3>
          </div>
          <div className="bg-white rounded-xl border border-[#c4d8d0] p-8 lg:p-10 pl-12">
            <p className="text-sm lg:text-base text-gray-600 leading-loose mb-6">{d.beyond.intro}</p>
            <ul className="space-y-3 mb-6">
              {d.beyond.conditions.map((c, i) => (
                <li key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-sm lg:text-base text-gray-600 leading-relaxed">{c}</span>
                </li>
              ))}
            </ul>
            <p className="text-sm lg:text-base text-gray-600 leading-loose pt-6 border-t" style={{ borderColor: `${ACCENT}20` }}>{d.beyond.closing}</p>
          </div>
        </div>
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className="text-xl lg:text-2xl font-serif font-bold text-gray-800 tracking-wide">{d.flow.title}</h3>
          </div>
          <p className="text-sm text-gray-500 pl-12 mb-8">在艾苜，每一帖水藥的誕生，都有我們嚴謹的流程把關：</p>
        ...
        <div className="bg-[#f2f7f5] rounded-xl p-6 border border-[#c4d8d0] mb-16" style={{ borderLeftWidth: '4px', borderLeftColor: ACCENT }}>
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs lg:text-sm text-gray-500 leading-relaxed">{d.disclaimer}</p>
          </div>
        </div>

        <TreatmentCasesSection cases={cases} accent="#5a7a6e" />
        ...
        <div className="rounded-3xl overflow-hidden relative" style={{ backgroundColor: ACCENT }}>
          <div className="relative z-10 py-16 px-10 text-center text-white">
            <h3 className="text-2xl lg:text-3xl font-serif font-normal mb-4">讓每一帖水藥，都成為您的深層修復</h3>
            <p className="text-base text-white/80 mb-8 max-w-xl mx-auto">現在就預約初診諮詢，讓艾苜的醫師為您梳理體質，開立最精準的水煎藥調理計畫。</p>
