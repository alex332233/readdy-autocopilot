...
const page = await client.fetch(`*[_type == "featuredTreatmentsPage"][0]{_id, cards[]{_key, title, image}}`)

if (!page?._id) {
  throw new Error('featuredTreatmentsPage not found')
}
...
for (const card of cards) {
  ...
  const seed = CARD_IMAGE_SEEDS[card?.title]
  if (!seed) continue
  ...
  patchedCount += 1
  console.log(`Seeded featured card image for ${card.title}`)
}

console.log(`Done. Seeded ${patchedCount} featured treatment card images.`)
